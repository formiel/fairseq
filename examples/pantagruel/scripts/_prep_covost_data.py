...
class CoVoST(Dataset):
    ...
    def __init__(
        ...
        for e in data:
            try:
                path = self.root / AUDIO_FOLDER / e["path"]
    ...
    def __getitem__(
        ...
        """
        data = self.data[n]
        path = self.root / AUDIO_FOLDER / data["path"].replace(".mp3", ".wav")
        rel_path = os.path.relpath(path.as_posix(), self.root)
        ...
        return rel_path, frames, translation
...
def process(args):
    ...
    # train_text = []
    for split in CoVoST.SPLITS:
        file_relpaths, frames, translations = [], [], []
        ...
        for rel_path, frame, translation in tqdm(dataset):
            print(f"rel_path: {rel_path}, frame={frame}, translation={translation}")
            file_relpaths.append(rel_path)
            frames.append(frame)
            translations.append(translation)

        # if "train" in split:
        #     train_text = translations
        ...
        with open(dest / f"{split}.tsv", "w") as tsv_out:
            print(root.as_posix(), file=tsv_out)
            for p, f in zip(file_relpaths, frames):
                print(
                    "{}\t{}".format(p, f), file=tsv_out
                )

        with open(dest / f"{split}.ltr", "w") as ltr_out, open(
            dest / f"{split}.wrd", "w") as wrd_out:
