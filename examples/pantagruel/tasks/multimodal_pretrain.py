...
from examples.pantagruel.data.raw_audio_augment import FileAudioAugmentDataset
...
@dataclass
class PantagruelMultimodalPretrainConfig(MultimodalPretrainingConfig):
...
@register_task("pantagruel_multimodal_pretrain", dataclass=PantagruelMultimodalPretrainConfig)
class PantagruelMultimodalPretrainTask(FairseqTask):
    cfg: PantagruelMultimodalPretrainConfig

    def __init__(self, cfg: PantagruelMultimodalPretrainConfig):
        super().__init__(cfg)
        self.audio_task = (
            AudioPretrainingTask(cfg.audio) if cfg.audio is not None and not cfg.musan_dir else None
        )
        self.image_task = (
    ...
    @classmethod
    def setup_task(cls, cfg: PantagruelMultimodalPretrainConfig, **kwargs):
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if self.cfg.musan_dir:
            musan_dir = Path(self.cfg.musan_dir)
            assert musan_dir.exists()
            logger.info(f"musan_dir: {musan_dir}")
            task_cfg = self.cfg.audio
            compute_mask = getattr(task_cfg, "precompute_mask_config", None) is not None
            mask_args = {}
            if compute_mask:
                mask_args = task_cfg.precompute_mask_config 
            ds = FileAudioAugmentDataset(
                manifest_path=os.path.join(task_cfg.data, "{}.tsv".format(split)),
                    sample_rate=task_cfg.get("sample_rate", 16000),
                    ...
                    text_compression_level=getattr(
                        TextCompressionLevel, str(task_cfg.text_compression_level)
                    ),
            ...
            ds = ModalityDatasetItem(
                datasetname=Modality.AUDIO,
                dataset=ds,
                max_positions=AudioPretrainingTask(task_cfg).max_positions(),
                max_tokens=self.cfg.max_tokens,
                max_sentences=None,
            )
            datasets.append(ds)
            self.mult_ratios.append(self.cfg.audio_ratio)
